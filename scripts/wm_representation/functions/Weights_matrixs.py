...
def Weights_matrix_Lasso( training_data, training_angles ):
    ...
    for voxel_x in range(0, n_voxels): #train each voxel
        ...
        lin = Lasso(alpha=0.001, precompute=True,  fit_intercept=False,  positive=False, selection='random') #no need to set normalize as it is ignored when intercept is set to False  
...
def Weights_matrix_Lasso_i( training_data, training_angles ):
    #####
    #### yes intercept
    #### also negative
    start_train_weights = time.time()
    ...
    for voxel_x in range(0, n_voxels): #train each voxel
        ...
        Y = training_data[:, voxel_x] ## Y is the real activity
        X = M_model ## X is the hipothetycal activit
        lin = Lasso(alpha=0.001, precompute=True,  fit_intercept=True,  positive=False, tol=0.00001, normalize=True, selection='random')   
        lin.fit(X,Y) # fits the best combination of weights to explain the activity
