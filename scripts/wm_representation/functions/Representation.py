...
#         channel= ch2vrep3(channel_36) ###Convert 36 into 720 channels for the reconstruction
#     ##
#     to_roll = - int( (angle_trial - ref)*(len(channel)/360) ) ## degrees to roll
#     channel=np.roll(channel, to_roll) ## roll this degrees
#     return channel
...
# def Representation_ccw(testing_data, testing_angles, Weights, Weights_t, ref_angle=180, plot=False, intercept=False):
#     ## Make the data parallelizable
#     n_trials_test = len(testing_data) #number trials
#     data_prall = []
#     for i in range(n_trials_test):
...
#     ###
#     numcores = multiprocessing.cpu_count()
#     Channel_all_trials_rolled = Parallel(n_jobs = numcores)(delayed(trial_rep_ccw)(Signal, angle_trial, Weights, Weights_t, ref=ref_angle, intercept_ = intercept)  for Signal, angle_trial in zip( data_prall, testing_angles))    ####
#     #Channel_all_trials_rolled = Parallel(n_jobs = numcores)(delayed(trial_rep_decode_trial_by_trial)(Signal, angle_trial, Weights, Weights_t, ref=ref_angle, intercept_ = intercept)  for Signal, angle_trial in zip( data_prall, testing_angles))    ####
#     Channel_all_trials_rolled = np.array(Channel_all_trials_rolled)
