...
for Subject in Subjects:
    for Brain_region in brain_regions:
        ...
        for trial in range(len(behaviour)):
            ...
            ### Training
            ###
            list_runs_train = behaviour.session_run.unique() 
            bool_l = list_runs_train!= session_trial
            training_runs = list(list_runs_train[bool_l])

            ## 
            Reconstructed_trials_leave1runout=[] ## nsessions x 16 x 720 matrix

            for training_run in training_runs:
                if cond_t == '1_7':
                    boolean_trials_training = np.array(behaviour['delay1']==7)  *  np.array(behaviour['order']==1) *  np.array(behaviour['session_run']==training_run)
                elif cond_t == '2_7':
                    boolean_trials_training = np.array(behaviour['delay1']==7)  *  np.array(behaviour['order']==2) *  np.array(behaviour['session_run']==training_run)
            #
                ...
                Reconstructed_trials_leave1runout.append(resconstr_trial)

            ##
            ##
            mean_reconstructed_trial = np.mean(Reconstructed_trials_leave1runout, axis=0) ## pass from nsessions x 16 x 720 matrix --> 16 x 720 matrix
            Reconstructed_trials.append(mean_reconstructed_trial)
        ##
        ##
