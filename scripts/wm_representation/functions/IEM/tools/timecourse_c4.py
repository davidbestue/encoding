...
def tiemcourse_c4(df_plot, title_plot, decoding_thing='target', dec_='decoding',  ylims=[-2,2]):
    ##
    ###
    ####   In the input dataframe you need the following columns:
    ###  'new_mean', 'inf', 'sup', 'brain_reg', 'time', 'condition'
    ## 
    pal = ['darkblue',  'darkorange',  'darkgreen', 'grey'] #sns.color_palette("tab10", n_colors=12, desat=1).as_hex()[0:3]
    ##
    ...
    for indx_c, condition in enumerate(['1_0.2', '1_7', '2_0.2', '2_7']): 
        ...
        sns.lineplot( ax=ax, x="times", y=dec_, hue='region', ci=95,
                     hue_order =  ['visual', 'ips', 'pfc', 'broca'], palette=pal, data=data_cond)
