...
def Representation_cv_angle_runsout(testing_activity, testing_behaviour, training_item, tr_st, tr_end, df_shuffle):
    ...
    for not_shared in list_wm_scans2:
        ...
        for train_index, test_index in zip(training_indexes, testing_indexes):
            ...
            rep_x = decoding_angles_pvector( testing_behaviour2=trials_test, testing_data=X_test, df_shuffle=df_shuffle, 
    ...
    #
    ####
    Reconstruction_not_shared = pd.DataFrame(Recons_trs)
    Reconstruction_not_shared.columns =  ['decoded_angle', 'target_centered', 'label_target', 'corresp_isolated', 'distractor_centered', 'corresp_isolated_distractor', 
        ...
        for Idx in reconstrction_x.new_index.unique():
            for Dec_item in ['T', 'NT1', 'NT2']:
                for Tr_ in reconstrction_x.TR_.unique():                    
                    ...
                    TR_x = str(df_x.TR_.iloc[0] * TR)
                    #
                    Recons_trs_shared.append([decoded_angle_, target_centered_, label_target_, corresp_isolated_, distractor_centered_, corresp_isolated_distractor_, 
                                                 label_distractor_, new_index_, TR_x])
            #
        #
    #
    ####
    Reconstruction_shared = pd.DataFrame(Recons_trs_shared)
    Reconstruction_shared.columns =  ['decoded_angle', 'target_centered', 'label_target', 'corresp_isolated', 'distractor_centered', 'corresp_isolated_distractor', 
                                'label_distractor', 'new_index', 'TR']
    #####
    #####
    Reconstruction = pd.concat([Reconstruction_shared, Reconstruction_not_shared], axis=0)
    #
