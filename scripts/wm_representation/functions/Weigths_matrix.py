...
def Weights_matrix_LM_i( training_data, training_angles ):
    ...
    for voxel_x in range(0, n_voxels): #train each voxel
        # set Y and X for the GLM
        Y = training_data[:, voxel_x] ## Y is the real activity
        X = M_model_zscored #_zscored #M_model ## X is the hipothetycal activity 
        ##
        X = sm.add_constant(X)
        a = sm.OLS(Y, X )
        resul = a.fit()
...
def Weights_matrix_LM_ni( training_data, training_angles ):
    #####
    ## Liniar model without fitting the intercept
    #####
    ## function to get Weight matrix
    ## trainiing data (trials, vx)
    ## training_angles (trials)
